...
const doTransform = (node)=>{
    if(node.childNodes && node.childNodes.length>0){
        //if there is v-if or v-for ,first transform it then foreach, because it will change childNodes.length
        let childNodes = [];
        node.childNodes.forEach(v=>{
            let vv = [v];
            //parse v-if and v-for
            if(v.tagName === 'template'){
                childNodes = directive.changeTemplate(v,childNodes)
            }else{
                childNodes = childNodes.concat(vv)
            }
        })
    ...
    if(node.attrs && node.attrs.length>0){
        node.attrs.forEach(v=>{
            ...
            if(v.name.includes('v-')){
                const {name,value} = directive.directive(v)
                v.name = name;
                v.value = value;
            }
        })
